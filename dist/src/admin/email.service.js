...
let EmailService = class EmailService {
    ...
    async sendEmail(emailOptions) {
        try {
            ...
            console.log(...oo_oo(`4214227199_37_6_37_66_4`, `Email sent successfully to ${emailOptions.to}`));
        }
        catch (error) {
            console.error(...oo_tx(`4214227199_39_6_39_51_11`, 'Failed to send email:', error));
            throw new Error('Failed to send email');
        }
    }
    async sendAssignmentResultEmail(to, studentName, assignmentTitle, marks, totalMarks, feedback, courseName, moduleName) {
    ...
    async sendPaymentSuccessEmail(to, studentName, courseName, amount, transactionId, paymentMethod) {
        const html = `
      <!DOCTYPE html>
      <html lang="en">
      <head>
    ...
    async sendEnrollmentWelcomeEmail(to, studentName, courseName, instructorName, facebookGroupLink, courseDuration, coursePrice) {
        const html = `
      <!DOCTYPE html>
      <html lang="en">
      <head>
    ...
    async sendPaymentFailedEmail(to, studentName, courseName, amount, reason) {
        const html = `
      <!DOCTYPE html>
      <html lang="en">
      <head>
    ...
    async sendGeneralNotificationEmail(to, studentName, title, message, actionUrl, actionText) {
        const html = `
      <!DOCTYPE html>
      <html lang="en">
      <head>
    ...
    async sendWelcomeEmail(to, studentName, verificationCode) {
        const html = `
      <!DOCTYPE html>
      <html lang="en">
      <head>
    ...
    async sendAdminWelcomeEmail(to, adminName, email, password, role) {
        const html = `
      <!DOCTYPE html>
      <html lang="en">
      <head>
